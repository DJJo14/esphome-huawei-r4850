...
void HuaweiR4850Component::setup() {
  ...
  automation = new Automation<std::vector<uint8_t>, uint32_t, bool>(
      canbus_canbustrigger);
  auto cb = [=](std::vector<uint8_t> x, uint32_t can_id,
                bool remote_transmission_request) -> void {
...
void HuaweiR4850Component::set_max_output_current(float value, bool offline) {
  ...
  if (offline)
    functionCode += 1;
  int32_t raw = R48XX_CURRENT_SCALLER * value;
  std::vector<uint8_t> data = {0x1,
                               functionCode,
                               0x0,
                               0x0,
...
void HuaweiR4850Component::set_power(bool value) {
  std::vector<uint8_t> data = {0x1, 0x32, 0x0, 0x00, 0, 0, 0, 0};
  if (value)
    data[3] = 0x00;
  else
    data[3] = 0x01;
...
void HuaweiR4850Component::on_frame(uint32_t can_id, bool rtr,
  ...
  if ((can_id & CAN_ID_MASK) == (CAN_ID_DATA & CAN_ID_MASK) ||
    ...
    case R48XX_DATA_SET_VOLTAGE:
      ...
      ESP_LOGI(TAG, "Voltage Conform %.2f %02X", conv_value, data[0] & 0xF0);
      if (!(data[0] & 0xF0))
    ...
    case R48xx_DATA_POWER_STATE:
      ESP_LOGI(TAG, "power state conform %08X", conv_value);
    ...
    case R48xx_DATA_OUTPUT_CURRENT1:
      ESP_LOGI(TAG, "Output Current? %.02fA %08X", value / 1024.0, value);
    ...
    default:
      ESP_LOGI(TAG, "Unknown parameter 0x%8X 0x%04X 0x%02X 0x%02X 0x%08X",
               can_id, signal_id, data[2], data[3], value);
      break;
    }
...
void HuaweiR4850Component::publish_sensor_state_(sensor::Sensor *sensor,
                                                 float value) {
  if (sensor) {
...
void HuaweiR4850Component::publish_number_state_(number::Number *number,
                                                 float value) {
  if (number) {
...
void HuaweiR4850Component::publish_switch_state_(switch_::Switch *pswitch,
                                                 const bool &state) {
  if (pswitch) {
